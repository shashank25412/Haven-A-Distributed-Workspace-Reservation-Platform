...
Reservation Reservation::create_confirmed(
    ...
    reservation.domain_events_.emplace_back(ReservationCreatedEvent{
        std::move(created_event_id),
        occurred_at,
    ...
    reservation.domain_events_.emplace_back(ReservationConfirmedEvent{
        std::move(confirmed_event_id),
        occurred_at,
        reservation.organization_id_,
        reservation.reservation_id_,
        reservation.resource_id_,
...
Reservation Reservation::create_pending_approval(
    OrganizationId organization_id,
    ReservationId reservation_id,
    ResourceId resource_id,
    UserId created_by,
    ...
    reservation.domain_events_.emplace_back(ReservationApprovalRequestedEvent{
        std::move(approval_requested_event_id),
        occurred_at,
        reservation.organization_id_,
        reservation.reservation_id_,
        reservation.resource_id_,
...
Reservation Reservation::rehydrate(
    OrganizationId organization_id,
    ReservationId reservation_id,
    ResourceId resource_id,
    UserId created_by,
...
void Reservation::record_persisted_version(const Version version) noexcept {
    version_ = version;
}

void Reservation::approve(UserId approved_by, const TimePoint approved_at, EventId confirmed_event_id) {
    HVN_TRACE_SCOPE();

    if (status_ != ReservationStatus::PendingApproval) {
        HVN_WARN_LOG("Reservation approval denied because the reservation is not pending approval.");
    ...
    domain_events_.emplace_back(ReservationConfirmedEvent{
        std::move(confirmed_event_id),
        approved_at,
        organization_id_,
        reservation_id_,
...
void Reservation::reject(UserId rejected_by, const TimePoint rejected_at, EventId rejected_event_id) {
    HVN_TRACE_SCOPE();

    if (status_ != ReservationStatus::PendingApproval) {
        HVN_WARN_LOG("Reservation rejection denied because the reservation is not pending approval.");
        throw std::logic_error("Only a pending reservation may be rejected.");
    }

    status_ = ReservationStatus::Rejected;

    domain_events_.emplace_back(ReservationRejectedEvent{
        std::move(rejected_event_id),
        rejected_at,
        organization_id_,
        reservation_id_,
        resource_id_,
...
void Reservation::cancel(UserId cancelled_by, const TimePoint cancelled_at, EventId cancelled_event_id) {
    HVN_TRACE_SCOPE();

    if (is_terminal(status_)) {
        HVN_WARN_LOG("Reservation cancellation denied because the reservation is already terminal.");
    ...
    domain_events_.emplace_back(ReservationCancelledEvent{
        std::move(cancelled_event_id),
        cancelled_at,
        organization_id_,
        reservation_id_,
        resource_id_,
...
void Reservation::extend(
    const TimePoint new_end,
    UserId extended_by,
    ...
    if (new_end <= interval_.end()) {
        HVN_WARN_LOG("Reservation extension denied because the new end time does not move forward.");
    ...
    domain_events_.emplace_back(ReservationExtendedEvent{
        std::move(extended_event_id),
        extended_at,
        organization_id_,
        reservation_id_,
        resource_id_,
...
void Reservation::expire(const TimePoint expired_at, EventId expired_event_id) {
    ...
    domain_events_.emplace_back(ReservationExpiredEvent{
        std::move(expired_event_id),
        expired_at,
        organization_id_,
        reservation_id_,
        resource_id_,
...
void Reservation::complete(const TimePoint completed_at, EventId completed_event_id) {
    ...
    domain_events_.emplace_back(ReservationCompletedEvent{
        std::move(completed_event_id),
        completed_at,
        organization_id_,
        reservation_id_,
        resource_id_,
