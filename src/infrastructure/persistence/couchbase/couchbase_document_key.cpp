...
constexpr std::string_view kOutboxDocumentPrefix{"outbox::"};
constexpr std::string_view kSeparator{"::"};

[[nodiscard]] std::string tenant_scoped_document_key(const std::string_view prefix,
                                                     const std::string_view organization_id,
                                                     const std::string_view entity_id) {
    std::string document_key;

    document_key.reserve(prefix.size() + organization_id.size() + kSeparator.size() +
                         entity_id.size());
...
std::string resource_document_key(const haven::domain::OrganizationId& organization_id,
                                  const haven::domain::ResourceId& resource_id) {
    return tenant_scoped_document_key(
        kResourceDocumentPrefix, organization_id.value(), resource_id.value());
}

std::string reservation_document_key(const haven::domain::OrganizationId& organization_id,
                                     const haven::domain::ReservationId& reservation_id) {
    return tenant_scoped_document_key(
        kReservationDocumentPrefix, organization_id.value(), reservation_id.value());
}

std::string outbox_document_key(const haven::domain::OrganizationId& organization_id,
                                const haven::domain::EventId& event_id) {
    return tenant_scoped_document_key(
        kOutboxDocumentPrefix, organization_id.value(), event_id.value());
}
