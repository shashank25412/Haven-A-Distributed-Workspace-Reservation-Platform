...
bool CouchbaseReservationCreationEventStore::contains_all(
    ...
    return metrics_.record(
        metrics::Repository::reservation_creation_event_store,
        metrics::Operation::contains_all,
        [&] {
            if (event_ids.size() != 2)
                return false;
            ...
            for (std::size_t index = 0; index < event_ids.size(); ++index) {
                ...
                try {
                    const auto document =
                        outbox_document_from_json(result.content_as<tao::json::value>());
                    ...
                    const auto creation_compatible =
                        index == 0
                            ? document.event_type == kReservationCreatedEventType
