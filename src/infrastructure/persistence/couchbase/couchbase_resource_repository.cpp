            ...
            try {
                ...
                if (resource.organization_id() != organization_id ||
                    ...
                    throw std::invalid_argument(
                        "Stored resource identity does not match its document key");
                }
                return haven::application::resources::LoadedResource{
                    std::move(resource), persistence_token_from(result.cas())};
...
CouchbaseResourceRepository::find_active_by_type(
    ...
    return metrics_.record(
        metrics::Repository::resource, metrics::Operation::find_active_by_type, [&] {
            HVN_TRACE_SCOPE();
            ...
            options.readonly(true)
                ...
                .named_parameters(
                    ...
                    std::make_pair("resourceType",
                                   std::string{haven::domain::to_string(resource_type)}));

            auto [error, result] =
                connection_->scope().query(active_resource_query(), options).get();
            if (error) {
            ...
            try {
                for (const auto& row : rows) {
                    ...
                    if (resource.organization_id() != organization_id ||
                        resource.type() != resource_type || !resource.is_active()) {
                        throw std::invalid_argument(
