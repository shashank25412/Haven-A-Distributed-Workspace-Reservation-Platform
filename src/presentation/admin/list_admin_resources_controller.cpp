/**
 * @file list_admin_resources_controller.cpp
 * @brief Implements GET /api/v1/admin/resources — lists all resources for the admin's organization.
 */
...
void handle(
    ...
    try {
        const auto resources = resource_repository->list_by_organization(
            haven::domain::OrganizationId{account.organization_id});

        Json::Value items{Json::arrayValue};
        for (const auto& resource : resources) {
            items.append(haven::presentation::resources::ResourceResponse{resource}.to_json());
        }
        Json::Value body;
    ...
    drogon::app().registerHandler(
        ...
        [resource_repository = std::move(resource_repository),
         authentication = std::move(authentication)](
            ...
            handle(resource_repository, authentication, request, std::move(callback));
        },
        {drogon::Get});
}
