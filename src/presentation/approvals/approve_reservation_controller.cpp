/**
 * @file approve_reservation_controller.cpp
 * @brief Implements the reservation approval HTTP route.
 */
...
constexpr char kRoute[]{"/api/v1/approvals/{reservationId}/approve"};
...
void handle(
    const std::shared_ptr<haven::application::reservations::ApproveReservationHandler>& handler,
    ...
    try {
        ...
        const haven::application::reservations::ApproveReservationCommand command{
        ...
        using enum haven::application::reservations::ApproveReservationStatus;
        switch (result.status()) {
            case APPROVED: {
                ...
                body["approvedAt"] =
                    reservations::reservation_http_timestamp(reservation.approval_info()->approved_at());
        ...
        HVN_ERROR_LOG("Reservation approval request failed unexpectedly");
        callback(error(request,
...
void register_approve_reservation_route(
    ...
    if (!handler)
        throw std::invalid_argument("Approve Reservation route handler must not be null");
    if (!authentication)
        throw std::invalid_argument("Approve Reservation authentication service must not be null");
