/**
 * @file list_decided_approvals_controller.cpp
 * @brief Implements the decided reservation approvals listing HTTP route.
 */
...
constexpr char kRoute[]{"/api/v1/approvals/history"};
...
void handle(
    ...
    try {
        ...
        const auto query = haven::application::reservations::ListDecidedApprovalsQuery{organization_id};
        ...
        for (const auto& reservation : reservations) {
            ...
            const auto is_rejected =
                reservation.status() == haven::domain::ReservationStatus::Rejected;
            item["status"] = is_rejected ? "REJECTED" : "APPROVED";
            ...
            if (is_rejected && reservation.rejection_info().has_value()) {
                ...
                decided_by["userId"] = Json::Value::null;
                decided_by["displayName"] = Json::Value::null;
                item["decidedAt"] = Json::Value::null;
                item["reason"] = Json::Value::null;
            }
            item["decidedBy"] = std::move(decided_by);
        ...
        HVN_ERROR_LOG("Decided approvals listing request failed unexpectedly");
        callback(error(request,
