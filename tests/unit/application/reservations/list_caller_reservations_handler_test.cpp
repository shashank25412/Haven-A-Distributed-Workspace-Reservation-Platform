...
[[nodiscard]] Reservation make_reservation(
...
TEST(ListCallerReservationsHandlerTest, Handle_ShouldReturnCallerReservations_WhenMatchesExist) {
    ...
    repository.add(make_reservation(
    ...
    const auto result = handler.handle(
        ListCallerReservationsQuery{organization_id, caller_id});
...
TEST(ListCallerReservationsHandlerTest, Handle_ShouldRemoveReservationsOutsideCallerScope_WhenRepositoryLeaksData) {
    const auto organization_id = OrganizationId{"organization-alpha"};
    const auto caller_id = UserId{"user-100"};
    const auto other_user_id = UserId{"user-200"};
